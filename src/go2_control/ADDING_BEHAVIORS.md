...
| `go2_control/voice_command_node.py` | Transcript -> normalized command; wake phrase |
...
### 1. Task Behaviors (FOLLOW, SEARCH)
...
For fire-and-forget gestures (like shake_hand):
...
3. Add handling in `request_voice_command()` (follow `_request_shake_hand()` pattern)
