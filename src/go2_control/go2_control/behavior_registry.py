...
class BehaviorDescriptor:
    ...
    motion_source : str | None
        Motion routing key: ``"nav"``, ``"follow"``, or ``None``.
...
BUILTIN_BEHAVIORS: tuple[BehaviorDescriptor, ...] = (
    ...
    BehaviorDescriptor(
        ...
        description="Cancels person-following and returns to IDLE.",
    ),
    BehaviorDescriptor(
        name="search",
        task_mode="SEARCH",
    ...
    BehaviorDescriptor(
        name="shake_hand",
        task_mode=None,
        voice_command="shake_hand",
        voice_phrases=("hello amigo",),
        voice_group="sport_test",
        requires_posture="STANDING",
        requires_idle=True,
        is_preemptible=False,
        operations=("publish_zero_motion", "send_hello"),
        deactivation_ops=(),
        motion_source=None,
        cooldown_exempt=False,
        description="Instant greeting gesture via Unitree Sport API.",
    ),
    BehaviorDescriptor(
        name="stand_up",
        task_mode=None,
    ...
    BehaviorDescriptor(
        name="explore",
        task_mode="EXPLORE",
        voice_command="explore",
        voice_phrases=("let's explore the area", "lets explore the area", "explore the area"),
        voice_group="explore",
        requires_posture="STANDING",
        requires_idle=False,
        is_preemptible=True,
        # No publish_zero_motion: the operator drives via the Unitree handheld
        # remote during a mapping session, so the supervisor stays off cmd_vel.
        operations=("start_explore",),
        deactivation_ops=(),
        motion_source=None,
        cooldown_exempt=False,
        description="Manual-drive SLAM mapping session that records ArUco marker positions.",
    ),
    BehaviorDescriptor(
        name="done_exploring",
        task_mode=None,
        voice_command="done_exploring",
        voice_phrases=("we are done exploring", "we're done exploring", "done exploring"),
        voice_group="explore",
        requires_posture=None,
        requires_idle=False,
        is_preemptible=False,
        operations=("publish_zero_motion", "save_explore_map"),
        deactivation_ops=(),
        motion_source=None,
        cooldown_exempt=True,
        description="Saves the explored map + ArUco marker table, returns to IDLE.",
    ),
    BehaviorDescriptor(
        name="deliver_swag",
        task_mode="DELIVER",
        voice_command="deliver_swag",
        voice_phrases=("deliver swag", "deliver the swag"),
        voice_group="deliver",
        requires_posture="STANDING",
        requires_idle=False,
        is_preemptible=True,
        operations=("publish_zero_motion", "start_deliver"),
        deactivation_ops=("publish_zero_motion", "cancel_deliver"),
        motion_source="nav",
        cooldown_exempt=False,
        description="Autonomous swag delivery: Nav2 + ArUco align to markers, run arm scripts.",
    ),
    BehaviorDescriptor(
        name="handoff_done",
        task_mode=None,
        voice_command="handoff_done",
        voice_phrases=("all done", "all finished"),
        voice_group="deliver",
        requires_posture=None,
        requires_idle=False,
        is_preemptible=False,
        operations=("signal_handoff_done",),
        deactivation_ops=(),
        motion_source=None,
        cooldown_exempt=True,
        description="Signals an in-progress delivery that the swag hand-off is complete.",
    ),
)
