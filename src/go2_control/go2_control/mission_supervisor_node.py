...
from go2_control.behavior_registry import get_behavior_by_voice_command
from go2_interfaces.action import ExploreArea, Search
...
class MissionSupervisorNode(Node):
    def __init__(self) -> None:
        ...
        self.attention_candidate_topic = str(
            self.declare_parameter("attention_candidate_topic", "/motion/candidate/attention").value
        )
        self.follow_candidate_topic = str(
            self.declare_parameter("follow_candidate_topic", "/motion/candidate/follow").value
        )
        self.hello_candidate_topic = str(
            self.declare_parameter("hello_candidate_topic", "/motion/candidate/hello").value
        )
        ...
        self.instant_hello_objects_topic = str(
            self.declare_parameter(
                "instant_hello_objects_topic", "/zed/zed_node/obj_det/objects"
            ).value
        )
        self.instant_hello_target_label = self._normalize_label(
            str(self.declare_parameter("instant_hello_target_label", "person").value)
        )
        self.instant_hello_min_target_confidence = max(
            0.0, float(self.declare_parameter("instant_hello_min_target_confidence", 40.0).value)
        )
        self.instant_hello_detection_timeout_s = max(
            0.1, float(self.declare_parameter("instant_hello_detection_timeout_s", 1.0).value)
        )
        ...
        self.explore_area_action_name = str(
            self.declare_parameter("explore_area_action_name", "explore_area").value
        )
        ...
        self._explore_area_goal_in_flight = False
        self._explore_area_cancel_requested = False
        self._active_explore_area_goal_handle = None
        ...
        self._latest_instant_hello_objects: ObjectsStamped | None = None
        self._latest_instant_hello_objects_received_mono_s: float | None = None

        self.search_client = ActionClient(self, Search, self.search_action_name)
        self.explore_area_client = ActionClient(self, ExploreArea, self.explore_area_action_name)
        ...
        self.create_subscription(
            Twist, self.attention_candidate_topic, self._attention_candidate_cb, 10
        )
        self.create_subscription(
            Twist, self.follow_candidate_topic, self._follow_candidate_cb, 10
        )
        self.create_subscription(Twist, self.hello_candidate_topic, self._hello_candidate_cb, 10)
        self.create_subscription(String, self.follow_event_topic, self._follow_event_cb, 10)
        self.create_subscription(
            ObjectsStamped,
            self.instant_hello_objects_topic,
            self._instant_hello_objects_cb,
            10,
        )
        ...
        self.get_logger().info(
            ...
            f"explore_action={self.explore_area_action_name} "
            f"follow_backend={self.follow_motion_backend} "
            f"attention_candidate_topic={self.attention_candidate_topic} "
            f"hello_candidate_topic={self.hello_candidate_topic} "
            f"instant_hello_objects_topic={self.instant_hello_objects_topic}"
        )
    ...
    def _cooldown_applies(self, command: str) -> bool:
        behavior = get_behavior_by_voice_command(command)
        if behavior is not None and behavior.cooldown_exempt:
            return False
        return command not in {"stop_follow", "lay_down", "stand_up", "hello_amigo"}
    ...
    def _voice_command_cb(self, msg: String) -> None:
        ...
        if command == "search" and not self._search_request_is_available():
            ...
            return
        if command == "explore_area" and not self._explore_area_request_is_available():
            self._emit_event(
                "voice_command_rejected",
                command=command,
                reason="explore_area_server_unavailable",
            )
            self.get_logger().warn(
                "Voice explore-area command rejected because the ExploreArea server is unavailable."
            )
            return
    ...
    def _handle_set_mode(self, request: SetRobotMode.Request, response: SetRobotMode.Response):
        ...
        requested_task_mode = str(request.task_mode).strip().upper()

        if requested_task_mode == TaskModes.SEARCH and not self._search_request_is_available():
            ...
            return response
        if requested_task_mode in {TaskModes.EXPLORE, "EXPLORE_AREA"} and not self._explore_area_request_is_available():
            response.accepted = False
            response.previous_task_mode = previous_state.task_mode
            response.previous_posture_mode = previous_state.posture_mode
            response.new_task_mode = previous_state.task_mode
            response.new_posture_mode = previous_state.posture_mode
            response.message = "ExploreArea server is unavailable."
            return response
    ...
    def _execute_operation(self, operation: str) -> None:
        ...
        if operation == SupervisorOps.CANCEL_SEARCH:
            ...
            return
        if operation == SupervisorOps.START_EXPLORE_AREA:
            self._start_explore_area()
            return
        if operation == SupervisorOps.CANCEL_EXPLORE_AREA:
            self._cancel_explore_area("supervisor_preempt")
            return
        if operation == SupervisorOps.SEND_HELLO:
            self._send_hello_gesture()
            return
        if operation == SupervisorOps.SEND_HELLO_IF_PERSON_VISIBLE:
            self._send_hello_if_person_visible()
            return
    ...
    def _explore_area_request_is_available(self) -> bool:
        state = self._current_state()
        if state.task_mode == TaskModes.EXPLORE:
            return True
        return self.explore_area_client.wait_for_server(timeout_sec=self.search_server_wait_s)
    ...
    def _start_explore_area(self) -> None:
        if self._explore_area_goal_in_flight or self._active_explore_area_goal_handle is not None:
            self._emit_event("explore_area_dispatch_skipped", reason="already_active")
            return

        if not self.explore_area_client.wait_for_server(timeout_sec=self.search_server_wait_s):
            self.get_logger().warn(
                f'ExploreArea action server "{self.explore_area_action_name}" is unavailable.'
            )
            self._replace_state(
                complete_explore_area(
                    self._current_state(),
                    detail="explore_area_server_unavailable",
                ),
                event_name="explore_area_dispatch_failed",
                detail="ExploreArea server unavailable.",
            )
            return
        ...
        self._explore_area_goal_in_flight = True
        self._explore_area_cancel_requested = False
        self._emit_event("explore_area_dispatch_started", action=self.explore_area_action_name)
        send_future = self.explore_area_client.send_goal_async(goal)
        send_future.add_done_callback(self._on_explore_area_goal_response)

    def _cancel_explore_area(self, reason: str) -> None:
        self._explore_area_cancel_requested = True

        if self._active_explore_area_goal_handle is None:
            self._emit_event(
                "explore_area_cancel_pending",
                reason=reason,
                waiting_for_goal_handle=self._explore_area_goal_in_flight,
            )
            return

        try:
            cancel_future = self._active_explore_area_goal_handle.cancel_goal_async()
            cancel_future.add_done_callback(self._on_explore_area_cancel_response)
            self._emit_event("explore_area_cancel_requested", reason=reason)
        except Exception as exc:
            self.get_logger().error(f"Failed to request ExploreArea cancel: {exc}")
            fault_state = enter_fault(self._current_state(), f"explore_area_cancel_failed: {exc}")
            self._replace_state(
                fault_state,
                event_name="explore_area_cancel_failed",
                detail=str(exc),
            )

    def _on_explore_area_cancel_response(self, future) -> None:
        try:
            cancel_response = future.result()
        except Exception as exc:
            self.get_logger().error(f"ExploreArea cancel response failed: {exc}")
            fault_state = enter_fault(
                self._current_state(),
                f"explore_area_cancel_response_failed: {exc}",
            )
            self._replace_state(
                fault_state,
                event_name="explore_area_cancel_response_failed",
                detail=str(exc),
            )
            return

        self._emit_event(
            "explore_area_cancel_response",
            goals_canceling=len(getattr(cancel_response, "goals_canceling", [])),
        )

    def _on_explore_area_goal_response(self, future) -> None:
        try:
            goal_handle = future.result()
        except Exception as exc:
            self._explore_area_goal_in_flight = False
            self._active_explore_area_goal_handle = None
            self.get_logger().error(f"Failed to send ExploreArea goal: {exc}")
            self._replace_state(
                complete_explore_area(
                    self._current_state(),
                    detail=f"explore_area_send_failed: {exc}",
                ),
                event_name="explore_area_send_failed",
                detail=str(exc),
            )
            return
        ...
        if goal_handle is None or not goal_handle.accepted:
            ...
            self._replace_state(
                complete_explore_area(
                    self._current_state(),
                    detail="explore_area_goal_rejected",
                ),
                event_name="explore_area_goal_rejected",
                detail="ExploreArea goal rejected.",
            )
            return

        self._active_explore_area_goal_handle = goal_handle
        self._emit_event("explore_area_goal_accepted", action=self.explore_area_action_name)
        if self._explore_area_cancel_requested:
            self._cancel_explore_area("pending_preempt_after_accept")
        result_future = goal_handle.get_result_async()
        result_future.add_done_callback(self._on_explore_area_result)

    def _on_explore_area_result(self, future) -> None:
        self._active_explore_area_goal_handle = None
        self._explore_area_goal_in_flight = False
        self._explore_area_cancel_requested = False
        ...
        try:
            ...
            status = -1
            detail = f"explore_area_result_failed: {exc}"

        new_state = complete_explore_area(
            self._current_state(),
            detail=detail or f"explore_area_status_{status}",
        )
        self._replace_state(
            new_state,
            event_name="explore_area_finished",
            detail=detail or f"status={status}",
        )
    ...
    def _send_hello_if_person_visible(self) -> None:
        if not self._instant_hello_person_visible():
            self._emit_event(
                "instant_hello_skipped",
                reason="no_visible_person",
                detection_timeout_s=self.instant_hello_detection_timeout_s,
            )
            self.get_logger().info(
                "Instant hello skipped because no visible person was detected on the ZED objects topic."
            )
            return

        self._send_hello_gesture()
        self._emit_event("instant_hello_sent", api_id=ROBOT_SPORT_API_ID_HELLO)

    def _instant_hello_objects_cb(self, msg: ObjectsStamped) -> None:
        self._latest_instant_hello_objects = msg
        self._latest_instant_hello_objects_received_mono_s = time.monotonic()

    def _instant_hello_person_visible(self) -> bool:
        if self._latest_instant_hello_objects is None:
            return False
        if self._latest_instant_hello_objects_received_mono_s is None:
            return False
        if (
            time.monotonic() - self._latest_instant_hello_objects_received_mono_s
        ) > self.instant_hello_detection_timeout_s:
            return False

        for obj in self._latest_instant_hello_objects.objects:
            if self._normalize_label(str(obj.label)) != self.instant_hello_target_label:
                continue
            if float(obj.confidence) < self.instant_hello_min_target_confidence:
                continue
    ...
    def _attention_candidate_cb(self, msg: Twist) -> None:
        routing = select_motion_routing(
            self._current_state(),
            follow_backend=self.follow_motion_backend,
        )
        if routing.base_source == "attention":
            self._publish_base_motion(msg)
    ...
    def _hello_candidate_cb(self, msg: Twist) -> None:
        routing = select_motion_routing(
            self._current_state(),
            follow_backend=self.follow_motion_backend,
        )
        if routing.base_source == "hello":
            self._publish_base_motion(msg)
