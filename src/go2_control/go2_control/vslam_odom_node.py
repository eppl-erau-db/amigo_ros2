...
class OdomNode(Node):
    def __init__(self):
        ...
        self.odom_subscriber = self.create_subscription(Odometry, "/visual_slam/tracking/odometry", self.odom_callback, 10)
        self.odometry_publisher = self.create_publisher(Odometry, 'vslam/odom', 10)
        ...
        self.get_logger().info("vslam odom has started")
    ...
    def publish_odom(self):
        if self.robot_pose:
            ...
            odom_trans.transform.translation.z = self.robot_pose.pose.position.z * 0
            odom_trans.transform.rotation = self.robot_pose.pose.orientation
            self.broadcaster.sendTransform(odom_trans)
            self.transform_publisher.publish(odom_trans)
    ...
    def publish_odom_topic(self):
        if self.robot_odom:
            ...
            odom_msg.pose.pose.position.z = self.robot_odom.pose.pose.position.z * 0
